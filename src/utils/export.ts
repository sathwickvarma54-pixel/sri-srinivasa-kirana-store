...
export function exportToExcel(reportName: string, headers: string[], rows: any[][]) {
  ...
  // Setup sheet data
  const data = [
    [`Sri Venkateswara Kirana & General Store — ${reportName}`],
    [`Generated: ${dateStr}`],
    [], // Blank Row 3
...
export function exportToPDF(reportName: string, headers: string[], rows: any[][], totalRow?: string[]) {
  ...
  doc.setFontSize(22);
  doc.setTextColor(22, 101, 52); // Primary color deep green (#166534)
  doc.text("Sri Venkateswara Kirana & General Store", 15, currentY);
  ...
  // Draw Headers
  doc.setFillColor(22, 101, 52); // Primary deep green
...
function drawFooter(doc: jsPDF, pageWidth: number, pageHeight: number) {
  ...
  doc.setFontSize(8);
  doc.setTextColor(113, 128, 150);
  doc.text("Generated by Sri Venkateswara Kirana & General Store | Stock Management System", 15, pageHeight - 10);
  doc.text(
