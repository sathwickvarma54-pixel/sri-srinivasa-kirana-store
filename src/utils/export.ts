...
export function exportToExcel(reportName: string, headers: string[], rows: any[][]) {
  ...
  // Setup sheet data
  const data = [
    [`Sri Srinivasa Kirana & General Store — ${reportName}`],
    [`Generated: ${dateStr}`],
    [], // Blank Row 3
...
export function exportToPDF(reportName: string, headers: string[], rows: any[][], totalRow?: string[]) {
  ...
  doc.text("Sri Srinivasa Kirana & General Store", 15, currentY);
...
function drawFooter(doc: jsPDF, pageWidth: number, pageHeight: number) {
  ...
  doc.setFontSize(8);
  doc.setTextColor(113, 128, 150);
  doc.text("Generated by Sri Srinivasa Kirana & General Store | Stock Management System", 15, pageHeight - 10);
  doc.text(
