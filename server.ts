...
async function callOpenRouterWithKey(prompt: string, apiKeyToUse: string): Promise<any> {
  ...
  const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    headers: {
      ...
      "X-Title": "Sri Venkateswara Kirana & General Store"
    },
...
app.post("/api/insights/forecast", async (req, res) => {
  try {
    ...
    const prompt = `You are a retail inventory AI for an Indian Kiran store called "Sri Venkateswara Kirana & General Store".
...
async function setupServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Sri Venkateswara Kirana & General Store Server is running on port ${PORT}`);
  });
}
